...
# SERVER = "10.0.0.34"    	#socket.gethostbyname(socket.gethostname())	# Ultra
SERVER = "10.0.0.12"		# socket.gethostbyname(socket.gethostname())
...
def handle_client(conn, addr):
	...
	while connected:
		...
		if command == 'Status':
			m = f'Garage Open Status = { gpio_garage.garage_open}'
			print( m )
			send(conn, m )

		if command == 'Garage':
			print('Garage')
			#gpio_garage.command = 'garage'
			gpio_garage.garage()
